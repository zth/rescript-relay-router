...
@live
let makeLink = (~byStatusDecoded: TodoStatusPathParam.t, ~statuses: option<array<TodoStatus.t>>=?) => {
  open RelayRouter.Bindings
...
let makeLinkFromQueryParams = (~byStatusDecoded: TodoStatusPathParam.t, queryParams: queryParams) => {
  makeLink(~byStatusDecoded, ~statuses=?queryParams.statuses, )
}
...
@live
let getActiveSubRoute = (location: RelayRouter.History.location): option<[#Child]> => {
  let {pathname} = location
  if RelayRouter.Internal.matchPath("/todos/:byStatusDecoded", pathname)->Belt.Option.isSome {
      Some(#Child)
    } else {
    None
  }
...
@live
let useActiveSubRoute = (): option<[#Child]> => {
  let location = RelayRouter.Utils.useLocation()
  React.useMemo(() => {

...
module PreloadInsertingStream = {
  ...
  @send external onQuery : (t, ~id : string, ~response : option<'a>=?, ~final : option<bool>=?) => () = "onQuery"
    ...
    stream := ReactDOMServer.renderToPipeableStream(
      ...
      ReactDOMServer.renderToPipeableStreamOptions(
        ...
        ~onShellReady= () => {
          ...
          response
            ->Express.Response.setStatus(didError.contents ? 500 : 200)
          ...
          response
            ->Express.Response.asWritable
            ->writeToStream(`<!DOCTYPE html><html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta
      name="viewport"
